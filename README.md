...
這個專案透過Antigravity IDE設計n8n工作流的雛形，旨在節省每日整理廣告報表的時間。其自動從 Google Sheets 擷取資料，計算 CTR 與 CVR，並使用 Cerebras的API使用GPT模型生成洞察報告，最後寄送 Email。
...
2. **設定 Credentials**:
   ...
   - 點兩下 **Send Email Report** 節點，選擇你的 Gmail Credential，並在 `message` 欄位填入公式：`{{ $json.html }}`。
